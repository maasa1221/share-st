...
class MemoDetailScreen extends React.Component {
  ...
  componentWillMount() {
    ...
    const ProfileById = id =>
      this.props.profile.find(profile => profile.id.toString() === id);
  ...
  render() {
    return (
      <div>
        ...
        <Textout>プロフィール画像</Textout>
        <Proimg
          src={
            this.state.my_photo_bool == true
              ? `https://sapeetapp.s3-ap-northeast-1.amazonaws.com/uploads/image${this.state.id}.jpg`
              : tempAvatar
          }
        />
